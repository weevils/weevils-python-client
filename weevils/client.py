...
class NotLoggedIn(Exception):
    def __init__(self, method_name):
        super(Exception, self).__init__("You must log in with a token before using the %s method" % method_name)
...
class WeevilsClient:
    ...
    def refresh_token(self, refresh_token, client_secret):
        ...
        token = self._session.refresh_token(
            token_url, refresh_token=refresh_token, client_id=self.client_id, client_secret=client_secret
        )
        return token
